            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class=""><a href="{{ url('admin/jadwal') }}" class="btn btn-outline-warning btn-sm">Kembali</a></li>
        ...
        <form action="{{ url('admin/jadwal') }}" method="POST">
            ...
            <div class="form-group row">
                ...
                <div class="col-6">
                    ...
                    <input required class="form-control @error('pelaksana') is-invalid @enderror" type="text" name="pelaksana" value="{{ old('pelaksana') }}">
                    @error('pelaksana')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-3 mt-2">
                    ...
                    <input required class="form-control @error('tempat') is-invalid @enderror" type="text" name="tempat" value="{{ old('tempat') }}">
                    @error('tempat')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-3 mt-2">
                    ...
                    <select required class="form-control @error('nama') is-invalid @enderror" name="jenis" id="jenis">
                        ...
                        @foreach($jenis as $j)
                            <option @if(old('jenis')==$j->nama) {{ "selected" }} @endif value="{{ $j->nama }}">{{ $j->nama }}</option>
                        @endforeach
                    </select>
                    @error('diklat_id')
                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <button type="submit" class="btn btn-pink">Tambahkan </button>
        </form>
    </div>
